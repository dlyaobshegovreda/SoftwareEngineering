...
Выведены все элементы первого множества, которых нет во втором
...
Основное отличие между set() и frozenset() заключается в том, что set() является изменяемым объектом, а frozenset() - нет. Это означает, что мы можем добавлять или удалять элементы из set(), но не можем этого сделать с frozenset().
...
Основная идея решения заключается в том, чтобы получить первый и последний элементы списка, затем поменять их местами и вернуть обновленный список.
...
Основная идея решения заключается в применении оператора "среза" к входному списку, чтобы извлечь необходимые элементы. 
...
Используются встроенные функции Python max() и len() для вычисления требуемого значения в одну строку. Далее показаны примеры использования функции useless() с различными входными списками.
...
Используется синтаксис распаковки списка, когда три переменные nikolay, vasiliy и ivan присваиваются элементам списка superheroes в порядке их следования. Затем выводится, какой супергерой достался каждому из ребят.
...
Отсортирован список и удалено значение при помощи pop()
...
Функция superset() принимает два множества в качестве аргументов и выводит соответствующее сообщение на основе результата сравнения.
...
Программа использует срезы списка для представления его в обратном порядке.
...
Программа считывает список посещений, определяет количество выданных чеков, количество разных посетителей и номер работника, который посещал ресторан чаще всех.
...
Программа сортирует список результатов бега, выводит три лучших и три худших результата, а также все результаты, начиная с 10-го.
...
Для решения задачи используется формула Герона для вычисления площади треугольника по трем сторонам. Сначала находятся максимальные и минимальные элементы в каждом из списков, затем вычисляется полупериметр для максимальных и минимальных значений, и, наконец, с помощью формулы Герона, рассчитываются площади двух треугольников.
...
Для решения этой задачи используется функция modify_grades, которая принимает список оценок в качестве аргумента и возвращает новый список с применением необходимых преобразований. 
...
Используется функция list_to_set, которая принимает список в качестве аргумента и возвращает соответствующее множество. Внутри функции сначала создается пустое множество result_set и словарь counts, который хранит количество повторений каждого числа.
Затем функция проходит по списку, обновляя счетчик повторений в словаре counts. После этого она добавляет в множество result_set как уникальные числа, так и их строковые представления с количеством повторений.

## Общие выводы по теме
В ходе выполнения данных самостоятельных работ я освоил базовые навыки работы с основными коллекциями данных в Python - массивами, списками и множествами.